...
## Типизация
Как обычно бывает с этим проектом, не укладываюсь в дедлайн, поэтому отправляю на суд то что есть. Сейчас добавила типы только к серверной части, клиентскую еще сделать не успела.
Перед запуском сервера нужно его скомпилировать командой `make server-build`. Затем сервер можно запустить командой `make server-start`.
