...
## Локализация
Ссылка на проект в gitlocalize: https://gitlocalize.com/repo/4668
При первом запуске приложения определяется язык браузера или выбирается язык по умолчанию. Затем на сервер делается запрос для получения локалей. Полученные локали и выбранный язык записываются в стор (да, плохо, но я пока не смогла придумать другое решение, где хранить локали). Если мы нажимаем на смену языка, происходит проверка, есть ли уже такие локали. Если есть, используем их. Если нет, снова запрашиваем с сервера и записываем в стор.